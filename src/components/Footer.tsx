import { ShieldCheck, Phone, Mail, Award, Landmark, ExternalLink } from "lucide-react";
...
export default function Footer() {
  ...
  return (
    <footer id="sage-footer" className="bg-charcoal-900 text-gray-400 pt-20 pb-12 border-t border-charcoal-800">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10 mb-16">
          ...
          <div className="col-span-2 lg:col-span-1 space-y-6">
            ...
            {/* Official Sage Logo */}
            <svg
              className="w-16 h-9 text-sage-600"
              viewBox="0 0 57 32"
              fill="none"
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              Sage Business Cloud Intacct delivers #1 ranked financial management software to midmarket operators worldwide, combining core ledgers with advanced multidimensional analytics.
            </p>
            <div className="space-y-2 text-xs">
              <a href="tel:18774372991" className="flex items-center space-x-2 text-sage-600 hover:text-sage-500 font-semibold">
                <Phone className="w-4 h-4" />
                <span>1-877-437-2991</span>
              </a>
              <span className="flex items-center space-x-2 text-gray-500 font-mono">
                <Mail className="w-4 h-4 text-gray-600" />
                <span>info@sageintacct.com</span>
              </span>
            </div>
          </div>
                ...
                {cat.links.map((link, lIdx) => (
                  <li key={lIdx}>
                    <a
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const formSection = document.getElementById("sage-form");
                        if (formSection) formSection.scrollIntoView({ behavior: "smooth" });
                      }}
                      className="hover:text-white transition-colors duration-200 flex items-center space-x-1 group"
                    >
                      <span>{link}</span>
                    </a>
                  </li>
        ...
        {/* Legal Disclaimers & Auditing Compliance Info */}
        <div className="border-t border-charcoal-800 pt-10 mt-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center text-xs">
          
          {/* Copyright text */}
          <div className="space-y-2">
            <p>© 2026 Sage Group plc or its licensors. All rights reserved.</p>
            <p className="text-gray-600 leading-relaxed text-[11px]">
              Sage, Sage logos, and Sage product and service names mentioned herein are registered trademarks or trademarks of Sage Group plc or its licensors. AICPA, G2, IDC MarketScape, and other trademarks are the property of their respective owners.
            </p>
          </div>

          {/* Core Trust Seals and legal links */}
          <div className="flex flex-col sm:flex-row sm:justify-end items-center gap-6">
            
            {/* Certifications and Compliance icons */}
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-1.5 text-gray-500 hover:text-sage-600 transition-colors cursor-pointer">
                <ShieldCheck className="w-5 h-5 text-sage-600" />
                <span className="text-[10px] font-mono uppercase">SOC 2 TYPE II</span>
              </div>
              <div className="flex items-center space-x-1.5 text-gray-500 hover:text-sage-600 transition-colors cursor-pointer">
                <Landmark className="w-5 h-5 text-sage-600" />
                <span className="text-[10px] font-mono uppercase">AICPA PREFERRED</span>
              </div>
            </div>

            {/* Sitemap links */}
            <div className="flex flex-wrap gap-4 text-[11px] text-gray-500 font-semibold justify-center">
              <a href="#" onClick={(e) => e.preventDefault()} className="hover:text-sage-600 transition-colors">Privacy Policy</a>
              <a href="#" onClick={(e) => e.preventDefault()} className="hover:text-sage-600 transition-colors">Terms of Use</a>
              <a href="#" onClick={(e) => e.preventDefault()} className="hover:text-sage-600 transition-colors">Cookie Settings</a>
            </div>
